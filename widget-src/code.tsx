...
// This component displays the card type label with a fill color.
  ...
  return (
    <AutoLayout padding={padding} fill={fillColor} cornerRadius={radius}>
      <Text
        ...
        letterSpacing={0.2}
        fontSize={18}
        fill={cardLabelTextColors[label as CardType]}
        >
          {label}
      </Text>
    </AutoLayout>
...
// This component displays the status label with a color dot.
function CardStatusLabel({label, color} : { label: string, color: string }) {
  return (
    <AutoLayout verticalAlignItems="center" spacing={10} padding={{ top: 4, bottom: 0, left: 0, right: 0 }}>
      <Text fontWeight={'bold'} fontSize={18} fill="#333" >{label}</Text>
  ...
  return (
    <AutoLayout hidden={!hideChildren}>
      <AutoLayout
        ...
        opacity={0.75} horizontalAlignItems="center" padding={{ top: 2 }} width={24} height={18} fill={color} cornerRadius={{ topLeft: 0, topRight: 0, bottomLeft: 24, bottomRight: 24 }}>
...
// This component displays a note input field based on the card type.
...
// This component displays debug information, such as the widget node ID and runtime debug info.
function DebugInfo() {
  const [runtimeDebugInfo, setRuntimeDebugInfo] = useSyncedState('runtimeDebugInfo', "");
  const [box,] = useSyncedState('box', "");
  return (
    <AutoLayout direction="vertical" padding={{ top: 16, right: 24, left: 24, bottom: 16 }}>
...
// This component displays a list of links with options to add, edit, and remove links.
function Links({ cardType }: { cardType: CardType }) {
  ...
  // If there are no links, return an empty AutoLayout to avoid rendering.
  return (
    <AutoLayout hidden={links.length <= 0 } width='fill-parent' direction="vertical" padding={{top: 0, left:24, right:24, bottom:32 }} spacing={12}>
      {
        links.map(l => {
          ...
          if (linksInEditing) {
            ...
            return (
              ...
              >
                ...
                `} />
                <Text fill={cardLinkColors[cardType]}>{l.text}</Text>
              </AutoLayout>
            )
          }
        }
      ...
      {
        linksInEditing == true &&
        <AutoLayout width={'fill-parent'} direction='vertical' spacing={24}>
        ...
        `} />
            <Text positioning='absolute' x={24} fill={'#69757A'} hoverStyle={{ fill: '#333A3D' }}>Add a link</Text>
          </AutoLayout>
          <AutoLayout
            fill={'#333A3D'}
            hoverStyle={{ fill: '#333' }}
...
function Widget() {
  ...
  return (
    <AutoLayout direction="vertical" horizontalAlignItems="center" padding={{ bottom: 0 }}>
      <AutoLayout
        width={425}
        height="hug-contents"
        direction="vertical"
      ...
      >
        {cardStatus != "none" ?
          <AutoLayout verticalAlignItems="center" spacing="auto" width="fill-parent" padding={{ left: 0, top: 0, bottom: 0, right: 24 }} >
            <CardTypeLabel
